...
```mermaid
flowchart TB
  O[Orchestrator. Budget. Summaries only.] --> P[Planner. Writes steps.jsonl.]
  P --> T[Test implementer. tests/ only.]
  T --> R{New tests failing?}
  R -->|no| Stop1[STOP. Nothing was proven.]
  R -->|yes| C[Code implementer. app/ only.]
  C --> J[Rubric judge. Ten rows. No model.]
  J --> F[Final judge. Is it actually done?]
  F --> G{Gate}
  G -->|pass| Done[Stop]
  G -->|retry| C
  G -->|escalate| H[Human]
...
flowchart LR
  ...
  M2 --> Score[Ten rows + a gate]
  Score --> PR[Pull request, or an honest escalation]
```

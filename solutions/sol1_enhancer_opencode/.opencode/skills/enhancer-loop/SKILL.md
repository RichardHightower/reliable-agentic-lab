---
name: enhancer-loop
description: One poll-and-act step for the ticket enhancer. Enhances every open draft ticket that still needs work. A human LGTM is the only comment that marks a ticket ready. Use when invoked as enhancer-loop, typically from `task run` or `opencode run --command enhancer-loop`.
---

---
name: enhancer-loop
description: One poll-and-act step for the ticket enhancer. Enhances every open draft ticket that still needs work. A human LGTM is the only comment that marks a ticket ready. Use when invoked as /enhancer-loop from Copilot Chat Agent mode or Copilot CLI, typically from `task run`.
...
Spawn both with the `agent` tool, passing the custom agent's name
(`enhancer-judge` or `enhancer-doer`). They come from this plugin. Neither
holds `edit`, `runCommands`, or `agent`, so neither can write a file, run
`gh`, or spawn further agents.
...
check. VS Code Copilot has no built-in loop skill, so repeated polling comes
...
5. Spawn the `enhancer-judge` agent on the real ticket file, and parse its
   ...
   `python3 .github/plugins/ticket-enhancer/skills/enhancer-loop/scripts/check_fields.py '<judge json>'`.
   ...
   `python3 .github/plugins/ticket-enhancer/skills/enhancer-loop/scripts/check_fields.py --kind <kind> '<judge json>'`.
...
8. Record this poll, and check the exits.
   ...
   `python3 .github/plugins/ticket-enhancer/skills/enhancer-loop/scripts/check_stop.py '{"round":

...
Everything runs from `solutions/sol1_enhancer_vscode/`. This folder is
standalone.

## Step zero: confirm the three names

VS Code loads project skills from `.github/skills/` and custom agents from
`.github/agents/`. Copilot CLI does the same, from the directory it starts in.
...
Interactive check: open **this folder** as the VS Code workspace (File >
Open Folder). Switch Copilot Chat to Agent mode. Type `/`. You want
`/enhancer-loop`. The two custom agents show under the agent picker, or as
subagents the skill can spawn.
...
1. File > Open Folder on `solutions/sol1_enhancer_vscode`.
2. Copilot Chat, Agent mode.
3. `/enhancer-loop --repo ../../work/northwind-field-crm`
...
VS Code Copilot has no built-in loop skill, so nothing inside the process can
...
`debug.log` in this folder. VS Code actually runs those hooks, which is the
difference from the Grok Build port.

For live progress in the terminal, `task run` already uses Copilot CLI, which
streams as it works.
...
- Binary on PATH: `copilot` (VS Code Copilot CLI).
- Auth: GitHub Copilot.
- Prove the gates: `task checks`.
- Deploy: `task poll-forever`, [cron](../../labs/lab1_enhancer/cron/), Actions `ENHANCER_BACKEND=vscode`, webhook `AGENT_BACKEND=vscode`.

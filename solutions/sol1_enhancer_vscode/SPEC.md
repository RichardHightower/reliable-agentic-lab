# Spec. Lab 1. Ticket enhancer, as a VS Code agent plugin.
...
The artifact is a Visual Studio Code agent plugin: one skill and two custom
agents, under `.github/plugins/ticket-enhancer/`. It grooms every open ticket
in your fork, one poll at a time.

VS Code discovers project skills from `.github/skills/` and custom agents from
`.github/agents/`, not from a plugin directory on its own. This folder ships
three symlinks under those paths that point into the plugin. The plugin stays
the source of truth. See [IMPLEMENTATION_NOTES.md](IMPLEMENTATION_NOTES.md).
...
Both custom agents carry a read-only allowlist (`search/codebase`,
`search/usages`, `web/fetch`). Neither has `edit`, `runCommands`, or `agent`,
so neither can write a file, run `gh`, or spawn another agent. The
orchestrator is the parent Copilot agent. It holds the shell, writes the
ticket file, and runs `gh`.
...
Open this folder as the VS Code workspace, or run Copilot CLI from this
folder. Skills live relative to that root. See
...
Interactive: open this folder in VS Code, switch Copilot Chat to Agent mode,
and type `/enhancer-loop --repo ../../work/northwind-field-crm`.
...
VS Code Copilot has no built-in loop skill, so the skill cannot re-invoke
...
`ENHANCER_BACKEND` to `vscode`.

VS Code Copilot CLI on hosted runners needs a Copilot token. Prefer Claude

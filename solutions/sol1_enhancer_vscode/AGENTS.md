...
The artifact here is a VS Code agent plugin, `ticket-enhancer`, in
...
- `enhancer-judge` is a custom agent under
  ...
  It holds no `edit`, no `runCommands`, and no `agent` tool. A judge that
...
Run Copilot from this folder. VS Code and Copilot CLI discover
`.github/skills/` and `.github/agents/` from the workspace root, so a run
started at the lab repo root cannot see this skill. See

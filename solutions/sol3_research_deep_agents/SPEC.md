...
The glossary, front-matter, next-step, and study-table rows (`glossary_complete`, `glossary_exact`, `methods_present`, `conclusion_present`, `study_table`, `front_matter`, `next_step`, `cta_language`) sit behind `enforce_structure`, off by default. Every `task paper` lane runs with it on, offline included; only a direct call to `check()` in a test leaves it off.

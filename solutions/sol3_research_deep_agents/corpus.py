...
def _run(runner, args: list[str], cwd: Path) -> subprocess.CompletedProcess:
    return runner(args, cwd=str(cwd), text=True, capture_output=True, check=False)
...
def ingest_brain(
    ...
    *,
    ...
    """Copy a run's knowledge bundle into a brain git worktree.

    Off by default at the CLI. Never writes `main` or `master`.
    """
    ...
    if open_pr and result.get("commit"):
        pr = _run(
            ...
            [
                ...
                "create",
                "--title",
                f"ingest sol3 bundle onto {branch}",
                "--body",
                "Flywheel ingest from a Lab 3 paper run. Do not merge to main without review.",
            ],
            top,
        )
        if pr.returncode == 0:

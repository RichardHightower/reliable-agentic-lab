...
def _as_ticket(issue: dict) -> ticket_mod.Ticket:
    """A GitHub issue is a title and a body. The parser wants one document."""
    return ticket_mod.parse(
        f"# {issue['title']}\n\n{issue['body']}",
        ticket_id=str(issue.get("number") or issue.get("id") or ""),
    )
...
def run_github(number: int, *, budget: int, client: gh.GitHub | None = None) -> dict:
    ...
    try:
        verdict = criteria.judge(_as_ticket(issue))

...
def _run(work_dir: Path, topic: str, brains: list[Path]) -> paper.Run:
    return paper.Run(
        topic=topic,
        work_dir=work_dir,
        turns=object(),
        state=paper.State.load_or_new(work_dir, topic),
        brain=None,
        brains=brains,
        log=lambda *a: None,
    )
...
def test_corpus_pack_meta_reports_the_relevant_count(loop_brain, tmp_path):
    run = _run(tmp_path / "work", CREATINE_TOPIC, [loop_brain])

    meta = paper.corpus_pack(run)

    assert meta["hits"] == 20
    assert meta["relevant"] == 0
...
def test_the_scout_runs_on_the_thin_pack_and_skips_on_the_thick_pack(loop_brain, tmp_path):
    thin_run = _run(tmp_path / "thin", CREATINE_TOPIC, [loop_brain])
    paper.corpus_pack(thin_run)
    thin_meta = paper.scout(thin_run)
    assert thin_meta["skipped"] is False

    thick_run = _run(tmp_path / "thick", LOOP_TOPIC, [loop_brain])
    paper.corpus_pack(thick_run)
    thick_meta = paper.scout(thick_run)
    assert thick_meta["skipped"] is True

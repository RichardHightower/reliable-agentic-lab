...
def test_the_fixture_backend_makes_no_network_call(monkeypatch):
    ...
    miss = metadata.fetch_record(
        "https://example.invalid/nothing-here", FixtureBackend(), model_title="Kept"
    )
...
def test_cached_fetch_pays_for_a_url_once_per_work_directory(tmp_path, monkeypatch):
    ...
    def counting(url, backend, *, model_title=""):
        calls.append(url)
        return real_fetch(url, backend, model_title=model_title)

    monkeypatch.setattr(metadata, "fetch_record", counting)

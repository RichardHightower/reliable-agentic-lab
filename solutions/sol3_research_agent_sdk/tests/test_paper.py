...
# -- attribution: the verifier checks the cited source says the claim. #471
...
def test_a_quote_absent_from_the_source_loses_the_binding(work, turns):
    """`attributed()` drops the binding. The claim's own cited source does not
    say what the claim says, so it never reaches the model verifier at all."""
    ...
    run = prepared(work, WithBackend(claims=claims))
...
def test_a_claim_with_no_attributed_binding_is_dropped_and_logged(work, turns):
    ...
    run = prepared(work, WithBackend(claims=claims))
    paper.verify(run)
    payload = json.loads((Path(work) / "claims.json").read_text())
    assert payload["claims"] == []
    assert payload["dropped"], "the drop was never logged"
    assert payload["dropped"][0]["reason"]


def test_a_numeric_unimportant_claim_still_reaches_attribution(work, turns):
    ...
    it would never have reached the model at all."""
...
def test_an_unattributable_source_keeps_the_binding_and_notes_it(work, turns):
    """No backend on `run.turns` (every pre-#471 test double): nothing is
    fetched, nothing is checked, and the claim survives exactly as before."""
    run = prepared(work, turns())
    paper.verify(run)
    claims = json.loads((Path(work) / "claims.json").read_text())["claims"]
    assert claims, "the claim was dropped with nothing to check it against"


def test_two_urls_in_one_reply_stay_single_source(work, turns):
    """#471, ported to the SDK's shape. A claim cites exactly one
    `source_url` here, so the raw-URL-count trick that fooled DA's
    `corroborate()` cannot arise structurally. The same principle still
    holds: passing attribution is not corroboration. A claim whose own
    citation is attributed stays `unverified`, not `verified`, until a real,
    separate verifier turn actually agrees.
    """
    ...
    run = prepared(work, WithBackend(verdict="unclear", claims=claims))
    counts = paper.verify(run)
    saved = json.loads((Path(work) / "claims.json").read_text())["claims"]
...
def test_not_found_writes_the_queries_into_the_note(work, turns):
    ...
    run = prepared(work, Silent())
    paper.verify(run)
    claims = json.loads((Path(work) / "claims.json").read_text())["claims"]
    assert "creatine alternate wording" in claims[0]["verifier_excerpt"]
    assert "creatine site:example.org" in claims[0]["verifier_excerpt"]


def test_not_found_with_no_reported_queries_still_names_the_claim(work, turns):
    ...
    paper.verify(run)
    claims = json.loads((Path(work) / "claims.json").read_text())["claims"]
    assert "A thing is true" in claims[0]["verifier_excerpt"]

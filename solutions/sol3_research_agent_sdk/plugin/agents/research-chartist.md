...
`type` is one of `bar`, `line`, `grouped_bar`, `scatter`. Prefer `bar`. Use
`line` only for a series ordered in time or in a stated sequence. Use
`scatter` only when both axes are numeric measurements.
...
Return ONLY JSON. The first character is `{` and the last is `}`.

...
class Backend:
    ...
    def judge(self, *, repo: Path, prompt: str) -> DoerResult:
        """Offline default. Live backends override this to call the judge subagent.

        The classroom path (none, reference, scripted tests) has no model. The
        loop still calls this so `judge_done` is a real boolean, never None, on
        a green rubric. Unparseable is a fail; this default is valid JSON.
        """
        return DoerResult(output='{"done": true, "why": "offline backend; rubric is green"}')

...
`gates.decide` takes a `judge_done` argument and `implementer.py` passes it.
Unparseable JSON is `done=False`, never a pass. Answer as structured JSON,
because the whole point of a structured answer is that Python can read it.

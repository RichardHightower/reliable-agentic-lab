...
Copilot CLI isolation is a per-agent tools allowlist. The orchestrator skill
runs in the parent Copilot CLI agent, which holds write and the `agent` tool.
The two custom agents must not. A tools line that includes `edit`,
`execute`/`shell`, or `agent` removes the reason the loop can be trusted.
...
def main() -> int:
    ...
    for path in AGENTS:
        ...
        tools = _tools(front)
        lowered = [t.lower().split("/")[0] for t in tools]

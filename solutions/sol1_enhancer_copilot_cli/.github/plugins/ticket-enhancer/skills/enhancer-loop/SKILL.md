---
name: enhancer-loop
description: One poll-and-act step for the ticket enhancer. Enhances every open draft ticket that still needs work. A human LGTM is the only comment that marks a ticket ready. Use when invoked as /enhancer-loop from GitHub Copilot CLI, typically from `task run`.
...
holds `edit`, `execute`, or `agent`, so neither can write a file, run
...
check. Copilot CLI has no built-in loop skill, so repeated polling comes

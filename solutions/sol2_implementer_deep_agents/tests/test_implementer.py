...
"""
    ...
    # This role-table probe is independent of the run above and reads the
    # original repo's own .loop.yml, not the worktree's.
...
def test_non_git_target_raises_contract_error(tmp_path):
    """git worktree add fails on a plain directory. Every other test in this
    file uses _git_repo; conftest.py's own `target_repo` fixture writes a
    flow-style Taskfile that fails Contract.validate() before this even runs,
    so this builds the plain directory locally instead."""
    ...
    with pytest.raises(implementer.ContractError, match="not a git repository"):
        implementer.run(repo=plain, ticket_id="T001", doer=doers.NoneBackend())

...
`SPEC.md` claims six things this port enforces: no `Bash` on any role, a
subagent tool list replaces the parent list, the judge holds `read_file`
only, `virtual_mode` is routing and not a security boundary, orchestrator
`permissions` deny writes, and the general-purpose subagent is off. Each
claim is written against one identifier a reader can grep in `roles.py`, so
the section cannot be deleted or drift silently: delete it, and the first
test below fails; misspell one identifier, and it fails too.
"""

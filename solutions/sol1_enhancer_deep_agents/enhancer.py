...
The doer holds a write tool scoped to `tickets/**` by `roles.py`, so it writes
its own candidate file. The judge is handed no write tool at all, which is why
it cannot grade its own draft. Both the backend and the GitHub wrapper are
constructor arguments, so the entire loop runs against fakes with no runtime,
...
class Enhancer:
    ...
    def draft(
        ...
        """Have the doer write a candidate. It writes the file, its tool scopes it.

        The doer is the only role here holding a write tool, and the scope
        check inside that tool (`roles.scoped_write_tool`) keeps it within
        `tickets/**`. The candidate path is inside that scope, so the write
        lands and everything else is still refused, which is the whole reason
        this port exists.
        """
    ...
    def _one(self, tkt: ticket_mod.Ticket, simulate_comment: str | None) -> Outcome:
        ...
        issue = (
            state.github_issue
            or (int(recorded) if recorded else None)
            or self.gh.find_issue(tkt.id)
        )

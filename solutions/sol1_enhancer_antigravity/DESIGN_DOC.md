---
title: "Lab 1 Antigravity Ticket Enhancer: Software Design Document"
document_type: "arc42 Software Design Document"
status: "Current implementation"
---

# Lab 1 Antigravity Ticket Enhancer
...
This solution is the Google Antigravity plugin port of the Lab 1 ticket
...
The implementation is rooted at `.agents/plugins/ticket-enhancer`. The plugin
has its own role instructions and calls the local check scripts from the
plugin directory. Workspace discovery is three symlinks under `.agents/skills/`
and `.agents/agents/`, because Antigravity does not auto-load a plugin folder.

```mermaid
flowchart LR
    Task["Task command"] --> Plugin["Antigravity plugin"]
    Plugin --> Orchestrator["Orchestrator skill"]
    Orchestrator --> Doer["Doer custom subagent"]
    Orchestrator --> Judge["Judge custom subagent"]
...
```text
sol1_enhancer_antigravity/
├── .agents/plugins/ticket-enhancer/  Antigravity plugin, roles, skill
├── .agents/skills/                   Workspace registration symlink
├── .agents/agents/                   Workspace registration symlinks
...
| --- | --- |
| `.agents/plugins/ticket-enhancer` | Defines the runtime-visible orchestration, doer, and judge behavior. |
...
sequenceDiagram
    ...
    participant J as Antigravity judge
    participant D as Antigravity doer
...
use requires Antigravity or `agy`, GitHub authentication, and a
configured target. The plugin does not own time-based scheduling.

| Quality scenario | Expected behavior |
| --- | --- |
| A role tries to invoke `replace_file_content`, `run_command`, or `invoke_subagent` | The role configuration refuses the attempt. `task inspect` fails if the allowlist includes them. |
...
Key risks are cwd assumptions, Antigravity availability, GitHub
...
| Term | Meaning |
| --- | --- |
| Plugin | Antigravity plugin folder: `plugin.json`, portable skills, client agents. |
| Custom subagent | Antigravity `.md` file with a tools allowlist. Spawned via `invoke_subagent`. |

# Spec. Lab 1. Ticket enhancer, as a Google Antigravity plugin.
...
The artifact is a Google Antigravity plugin: one skill and two custom
subagents, under `.agents/plugins/ticket-enhancer/`. It grooms every open ticket
in your fork, one poll at a time.

Antigravity discovers workspace skills from `.agents/skills/` and custom subagents from
`.agents/agents/*.md`, not from a plugin directory on its own. This folder ships
three symlinks under those paths that point into the plugin. The plugin stays
the source of truth. Tool names are Antigravity's (`view_file`, `grep_search`,
`replace_file_content`, `run_command`, `invoke_subagent`). See [IMPLEMENTATION_NOTES.md](IMPLEMENTATION_NOTES.md).
...
| Role | Kind | Writes? |
|---|---|---|
| `enhancer-judge` | custom subagent (`.md`) | no |
| `enhancer-doer` | custom subagent (`.md`) | no |
...
Both custom subagents carry a read-only allowlist (`view_file`, `grep_search`).
Neither has `replace_file_content`, `run_command`, or `invoke_subagent`, so neither
can write a file, run `gh`, or spawn another agent. They set `subagent: true`,
`mainAgent: false`, and `commandExecutionPolicy: off`. The orchestrator is the
parent Antigravity agent. It holds the shell, writes the ticket file, and runs `gh`.
...
Open this folder in Antigravity, or run `agy` from this folder. Skills live
relative to that root. See [IMPLEMENTATION_NOTES.md](IMPLEMENTATION_NOTES.md).
...
Interactive: open this folder in Antigravity and type `/enhancer-loop --repo ../../work/northwind-field-crm`.
...
Antigravity has no built-in loop skill, so the skill cannot re-invoke
...
- `.agents/plugins/ticket-enhancer/skills/enhancer-loop/SKILL.md`
- `.agents/plugins/ticket-enhancer/agents/enhancer-judge.md`
- `.agents/plugins/ticket-enhancer/skills/enhancer-loop/scripts/check_fields.py`
...
`ENHANCER_BACKEND` to `antigravity`.

Antigravity CLI on hosted runners needs an authenticated `agy` install. Prefer Claude

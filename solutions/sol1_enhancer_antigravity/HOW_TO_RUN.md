...
Everything runs from `solutions/sol1_enhancer_antigravity/`. This folder is
standalone.

## Step zero: confirm the three names

Antigravity loads workspace skills from `.agents/skills/` and custom subagents from
`.agents/agents/`, from the directory it starts in.
...
Interactive: open **this folder** in Antigravity. Type `/`. You want
`/enhancer-loop`. The two custom subagents are spawned by the skill, they
are not primary agents (`mainAgent: false`).

Headless check: `task inspect`, then `task run --` from this folder.
...
Interactive instead of headless: open this folder in Antigravity and type
...
Antigravity has no built-in loop skill, so nothing inside the process can
...
`.agents/plugins/ticket-enhancer/hooks.json` append one line per tool call to
`debug.log` in this folder when the host runs them.

`task run` already uses `agy -p`, which streams as it works.

---
name: enhancer-loop
description: One poll-and-act step for the ticket enhancer. Enhances every open draft ticket that still needs work. A human LGTM is the only comment that marks a ticket ready. Use when invoked as /enhancer-loop from Antigravity IDE or `agy`, typically from `task run`.
argument-hint: "[--repo <path>] [--ticket Txxx] [--simulate-comment text]"
---
...
Spawn both with the `invoke_subagent` tool, passing the custom agent's name
(`enhancer-judge` or `enhancer-doer`). They come from this plugin. Neither
holds `replace_file_content`, `run_command`, or `invoke_subagent`, so neither
can write a file, run `gh`, or spawn further agents.
...
check. Antigravity has no built-in loop skill, so repeated polling comes
from outside this process: `task poll-forever`, a cron job, or a scheduled
...
5. Spawn the `enhancer-judge` agent on the real ticket file, and parse its
   ...
   `python3 .agents/plugins/ticket-enhancer/skills/enhancer-loop/scripts/check_fields.py '<judge json>'`.
   ...
   `python3 .agents/plugins/ticket-enhancer/skills/enhancer-loop/scripts/check_fields.py --kind <kind> '<judge json>'`.
...
8. Record this poll, and check the exits.
   ...
   `python3 .agents/plugins/ticket-enhancer/skills/enhancer-loop/scripts/check_stop.py '{"round":

...
Build the ticket enhancer as a Google Antigravity plugin. The finished answer
is `solutions/sol1_enhancer_antigravity/`. Read its
[SPEC.md](../../../solutions/sol1_enhancer_antigravity/SPEC.md) for the design
and
[IMPLEMENTATION_NOTES.md](../../../solutions/sol1_enhancer_antigravity/IMPLEMENTATION_NOTES.md)
for how Antigravity loads a plugin, which is the part that will surprise you.
...
Headless: `agy --dangerously-skip-permissions -p "$(cat prompts/antigravity.md)"`.
Interactive: open this folder in Antigravity and paste each prompt below.
...
**Antigravity does not auto-load a plugin folder.** `.agents/plugins/ticket-enhancer/`
is the packagable unit. What Antigravity actually discovers are workspace
skills under `.agents/skills/` and custom subagents under `.agents/agents/*.md`.
...
workspace root. `agy` started at the repo root cannot see `/enhancer-loop`.
`task run` pins `dir:` for that reason.
...
judge or the doer gained `replace_file_content`, `run_command`, or
`invoke_subagent`, or if `subagent: true` / `mainAgent: false` /
`commandExecutionPolicy: off` drifted.

## Prompt 1: the plugin, and the three symlinks

Create the plugin under `.agents/plugins/ticket-enhancer/`:

```
.agents/plugins/ticket-enhancer/
  plugin.json                     Antigravity plugin.json, name ticket-enhancer
  skills/enhancer-loop/SKILL.md   the orchestrator, one poll-and-act step
  agents/enhancer-doer.md         drafts the ticket, writes nothing
  agents/enhancer-judge.md        grades against the rubric, writes nothing
```

Then register it with the three symlinks Antigravity actually reads:
...
doer, so check it before you go on. `tools:` is an allowlist: `view_file`,
`grep_search`. No `replace_file_content`. No `run_command`. No
`invoke_subagent`. Set `subagent: true`, `mainAgent: false`, and
...
Antigravity's subagent tool is `invoke_subagent`. Custom subagents live in
`agents/*.md`. `IMPLEMENTATION_NOTES.md` explains the allowlist and why a
misspelled tool name can hang a subagent.
...
`task run` starts Antigravity CLI from this folder:

```bash
agy --dangerously-skip-permissions -p "/enhancer-loop --repo <target> ..."
```
...
[FALL-BEHIND.md](../FALL-BEHIND.md) has the copy commands for the Antigravity tree.

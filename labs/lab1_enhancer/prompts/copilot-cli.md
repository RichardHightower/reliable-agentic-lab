...
Build the ticket enhancer as a GitHub Copilot CLI agent plugin. The finished
answer is `solutions/sol1_enhancer_copilot_cli/`. Read its
[SPEC.md](../../../solutions/sol1_enhancer_copilot_cli/SPEC.md) for the design
and
[IMPLEMENTATION_NOTES.md](../../../solutions/sol1_enhancer_copilot_cli/IMPLEMENTATION_NOTES.md)
for how Copilot CLI loads a plugin, which is the part that will surprise you.
...
Headless: `copilot --allow-all --prompt "$(cat prompts/copilot-cli.md)"`.
Interactive: `copilot` in this folder, then paste each prompt below.
...
**Copilot CLI does not auto-load a plugin folder.** `.github/plugins/ticket-enhancer/`
is the packagable unit. What Copilot CLI actually discovers are project
skills under `.github/skills/` and custom agents under `.github/agents/*.agent.md`.
Three symlinks under those paths are what make the loop runnable, and
prompt 1 writes them.

**Start Copilot in this folder, not the lab repo root.** Skills are discovered
from cwd. Copilot CLI started at the repo root cannot see `/enhancer-loop`.
...
judge or the doer gained `edit`, `execute`/`shell`, or `agent`.
...
Then register it with the three symlinks Copilot CLI actually reads:
...
doer, so check it before you go on. `tools:` is an allowlist of Copilot CLI
aliases: `read`, `search`. No `edit`. No `execute`. No `shell`. No `agent`.
...
Copilot CLI's subagent tool is `agent`. Custom agents live in
`com.github.copilot/agents/*.agent.md`. Tool names are the CLI aliases, not
the VS Code ids. `IMPLEMENTATION_NOTES.md` explains why.
...
[FALL-BEHIND.md](../FALL-BEHIND.md) has the copy commands for the Copilot CLI tree.
